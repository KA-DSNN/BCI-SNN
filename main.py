...
for subject in mcnn_training_data.keys():
    ...
    index_array = list(range(dataset.shape[0]))
    random.shuffle(index_array)
    
    train_data = np.array([dataset[x] for x in index_array[:504]])
    test_data = np.array([dataset[x] for x in index_array[504:]])
    train_labels = np.array([labels[x] for x in index_array[504:]])
    test_labels = np.array([labels[x] for x in index_array[504:]])
    ...
    for real_epoch in range(num_epochs):
        ...
        for epoch in range(1):
            ...
            for i, (images, label) in enumerate(zip(train_data, train_labels)):
                ...
                # for j in range(images2.shape[0]):
                #     # img0 = np.array(images2[j])
                ...
                # print(torch.zeros(batch_size * 2, 24).scatter_(1, labels2.view(-1, 1), 1))
                # print(torch.zeros(batch_size * 2, 24).scatter_(1, labels2.view(-1, 1), 1).shape)
                ...
                print(outputs)
                print(outputs.shape)
                labels_ = torch.zeros(batch_size * 2, 24).scatter_(1, labels2.view(-1, 1), 1)
        ...
        with torch.no_grad():
            for batch_idx, (images, label) in enumerate(zip(test_data, test_labels)):
                print("NUM: ", batch_idx)
                # print("image shape: ", images.shape)
                # print("label shape: ", label.shape)
                # pprint(label)

                images2 = torch.empty((1, 12, 28, 28))
                img_eeg = torch.from_numpy(np.reshape(images[:, :98, :], (28, 28)))
                for j in range(12):
                    images2[0, j] = img_eeg
                #images2 = torch.empty((images.shape[0] * 2, images.shape[1], images.shape[2]))
                ...
                # print("label number: ", labels2)
                # print("reshaped image shape: ", images2.shape)
        ...
        class_names = ['0_ccw', '1_ccw', '2_ccw', '3_ccw', '4_ccw',
                ...
                '5_cw', '6_cw', '7_cw', '8_cw', '9_cw', "10_cw", "11_cw"]
